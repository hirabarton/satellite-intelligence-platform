...
export default function CosmicNoirNetworkV2({
  width = 1024,
  height = 1024,
  ...
  return (
    ...
    >
      <svg viewBox={view} width="100%" height="100%" role="img">
        <defs>
          {/* Soft vignette */}
          <radialGradient id="vignette-v2" cx="50%" cy="40%" r="70%">
          ...
          {/* Star + node glow */}
          <radialGradient id="nodeGlow-v2" cx="50%" cy="50%" r="50%">
          ...
          <radialGradient id="earthBody-v2" cx="50%" cy="85%" r="85%">
            <stop offset="0%" stopColor="#021528" />
            <stop offset="60%" stopColor="#021528" />
            <stop offset="100%" stopColor="#000408" />
          </radialGradient>
          <radialGradient id="earthGlow-v2" cx="50%" cy="85%" r="80%">
          ...
          <linearGradient id="flow-v2" x1="0%" y1="0%" x2="100%" y2="0%">
          ...
          {/* Core prism glow */}
          <radialGradient id="core-v2" cx="50%" cy="50%" r="60%">
            <stop offset="0%" stopColor={glow} stopOpacity="1" />
            <stop offset="60%" stopColor={glow} stopOpacity="0.35" />
            <stop offset="100%" stopColor={glow} stopOpacity="0" />
          </radialGradient>

          <filter id="blur-3-v2" x="-30%" y="-30%" width="160%" height="160%">
            <feGaussianBlur stdDeviation="3" />
          </filter>
          <filter id="blur-8-v2" x="-30%" y="-30%" width="160%" height="160%">
        ...
        <rect x="0" y="0" width={width} height={height} fill="url(#vignette-v2)" />

        {/* Twinkling stars */}
        {twinkles.map((s, i) => (
          <circle key={i} cx={s.x} cy={s.y} r={s.r} fill="url(#nodeGlow-v2)" className={`twinkle tw-${i % 7}`} />
        ))}

        {/* Earth: visible body, glow, and lights */}
        <g>
          <ellipse cx={cx} cy={height * 0.9} rx={width * 0.85} ry={height * 0.28} fill="url(#earthBody-v2)" />
          <ellipse cx={cx} cy={height * 0.88} rx={width * 0.75} ry={height * 0.22} fill="url(#earthGlow-v2)" filter="url(#blur-8-v2)" />
          {lights.map((p, i) => (
        ...
        <g>
          {nodes.map((n, i) => {
            ...
            return (
              <path key={i} d={d} fill="none" stroke="url(#flow-v2)" strokeWidth={1.6} strokeLinecap="round" className="flow" style={{ strokeDasharray: `${Math.max(30, len * 0.18)} ${Math.max(100, len * 0.82)}`, animationDelay: `${(i % 7) * 0.35}s` }} />
            );
          })}
        </g>
        ...
        <g>
          {nodes.map((n, i) => (
            <g key={i}>
              <circle cx={n.x} cy={n.y} r={12} fill="url(#nodeGlow-v2)" opacity="0.9" />
        ...
        <g transform={`translate(${cx}, ${cy - 16})`}>
          <circle r={70} fill="url(#core-v2)" filter="url(#blur-8-v2)" opacity="0.9" />
          {[-48, -12, 24].map((y, i) => (
            <g key={i} transform={`translate(0, ${y})`}>
              <polygon points="0,-40 46, -16 0, 8 -46, -16" fill={glow} opacity="0.15" filter="url(#blur-3-v2)" />
